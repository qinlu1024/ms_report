...
sql_res = " SELECT P.ACTV_MON,P.TRD_MON,P.MON_DIF,P.BHDT_BCH_CDE,P.PRODUCT_TYP,P.LOAN_TYP, P.NBR, P.RES_AMT " \
...
sql_yql = " SELECT L.ACTV_MON,L.BHDT_BCH_CDE,L.PRODUCT_TYP,L.LOAN_TYP, D.TRD_MON , (D.RES_AMT/L.DIS_AMT) YQ " \
          " FROM LOAN_DIS_AMT L , LOAN_DUE_AMT D WHERE 1=1 " \
...
data_yql_p1 = pd.pivot_table(data_yql, values='YQ', columns='TRD_MON',
                             index=['ACTV_MON', 'BHDT_BCH_CDE', 'PRODUCT_TYP', 'LOAN_TYP'], fill_value=0)
# data_yql_p1.rename(index=org_dict, inplace=True)


sql_yqbs = " SELECT L.ACTV_MON,L.BHDT_BCH_CDE,L.PRODUCT_TYP,L.LOAN_TYP, D.TRD_MON , (D.NBR/L.NBR) YQ " \
          " FROM LOAN_DIS_AMT L , LOAN_DUE_AMT D WHERE 1=1 " \
          " AND L.ACTV_MON = D.ACTV_MON " \
          " AND L.BHDT_BCH_CDE = D.BHDT_BCH_CDE " \
          " AND L.PRODUCT_TYP = D.PRODUCT_TYP " \
          " AND L.LOAN_TYP = D.LOAN_TYP "
data_yqbs = pd.read_sql(sql_yqbs, engine)
# data_yql['BHDT_BCH_CDE'] = 'BSBK99' + data_yql['BHDT_BCH_CDE']
# data_yql.rename(columns=col_dict, inplace=True)
data_yql_p2 = pd.pivot_table(data_yqbs, values='YQ', columns='TRD_MON',
                             index=['ACTV_MON', 'BHDT_BCH_CDE', 'PRODUCT_TYP', 'LOAN_TYP'], fill_value=0)
...
with pd.ExcelWriter('D:\\test\\due_list28.xlsx') as writer:
    ...
    data_yql_p2.to_excel(writer, sheet_name='逾期分析笔数', na_rep='0', float_format="%.4f")
